...
export async function listClients(
  ...
  if (role === 'CLIENT') {
    const { rows } = await tx.query(`select * from "ClientCompany" where id = $1 limit 1`, [clientId]);
    return rows;
  }
  const { rows } = await tx.query(
    `select * from "ClientCompany" order by "createdAt" desc limit $1 offset $2`,
    [limit, offset],
  );
  return rows;
}
...
export async function getClient(tx: PoolClient, id: string) {
  const { rows } = await tx.query(`select * from "ClientCompany" where id=$1`, [id]);
  return rows[0] ?? null;
}

export async function createClient(tx: PoolClient, body: CreateClientBodyT, tenantId: string) {
  const { rows } = await tx.query(
    `insert into "ClientCompany"(id,"tenantId",name,domain,active) values (gen_random_uuid(),$1,$2,$3,$4) returning *`,
...
export async function mapEmployeeToClient(tx: PoolClient, userId: string, clientId: string, tenantId: string) {
  await tx.query(
    `insert into "UserClientMap"(id,"tenantId","userId","clientId") values (gen_random_uuid(),$1,$2,$3) on conflict ("userId","clientId") do nothing`,
