...
exports.getUserActivityMetrics = getUserActivityMetrics;
const errors_1 = require("../utils/errors");
/**
 * Get dashboard metrics scoped by user role
 */
function getDashboardMetrics(tx, organizationId, userRole, userId, clientId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const ticketConditions = ['t.is_deleted = false'];
        const ticketParams = [];
        let paramIndex = 1;
        if (userRole === 'ADMIN') {
        ...
        // Get ticket counts
        const { rows: ticketCounts } = yield tx.query(`SELECT
      COUNT(*)::int as total,
    ...
    WHERE ${ticketWhereClause}`, ticketParams);
        ...
        const { rows: byStatus } = yield tx.query(`SELECT
      s.id as status_id,
    ...
    ORDER BY s.sequence, s.name`, ticketParams);
        ...
        const { rows: byPriority } = yield tx.query(`SELECT
    ...
    ORDER BY pr.rank`, ticketParams);
        ...
        if (userRole === 'EMPLOYEE') {
            const { rows: assignedCount } = yield tx.query(`SELECT COUNT(*)::int as count
       FROM ticket t
       JOIN project p ON p.id = t.project_id
       JOIN client c ON c.id = p.client_id
       WHERE ${ticketWhereClause} AND t.assigned_to_user_id = $${paramIndex}`, [...ticketParams, userId]);
            metrics.tickets.assignedToMe = ((_a = assignedCount[0]) === null || _a === void 0 ? void 0 : _a.count) || 0;
        }
        // Get project counts
      ...
      WHERE pm.project_id = p.id AND pm.user_id = $${projectParamIndex}
        ...
        const { rows: projectCounts } = yield tx.query(`SELECT
      COUNT(*)::int as total,
      COUNT(*) FILTER (WHERE p.active = true)::int as active
    FROM project p
    JOIN client c ON c.id = p.client_id
    ${projectWhereClause}`, projectParams);
        ...
        if (userRole === 'ADMIN') {
            // Client counts
            const { rows: clientCounts } = yield tx.query(`SELECT
        COUNT(*)::int as total,
        COUNT(*) FILTER (WHERE active = true)::int as active
      FROM client
      WHERE organization_id = $1`, [organizationId]);
            metrics.clients = {
                total: ((_b = clientCounts[0]) === null || _b === void 0 ? void 0 : _b.total) || 0,
                active: ((_c = clientCounts[0]) === null || _c === void 0 ? void 0 : _c.active) || 0,
            };
            // User counts
            const { rows: userCounts } = yield tx.query(`SELECT
        COUNT(*)::int as total,
        COUNT(*) FILTER (WHERE is_active = true)::int as active
      FROM app_user
      WHERE organization_id = $1`, [organizationId]);
            metrics.users = {
                total: ((_d = userCounts[0]) === null || _d === void 0 ? void 0 : _d.total) || 0,
                active: ((_e = userCounts[0]) === null || _e === void 0 ? void 0 : _e.active) || 0,
            };
        }
        return metrics;
    });
 ...
 */
function getRecentActivity(tx_1, organizationId_1, userRole_1, userId_1, clientId_1) {
    return __awaiter(this, arguments, void 0, function* (tx, organizationId, userRole, userId, clientId, limit = 20) {
        ...
        const { rows: events } = yield tx.query(`SELECT
      te.id,
      te.event_type,
      te.ticket_id,
    ...
    LIMIT $${paramIndex}`, [...ticketParams, limit]);
        // Get recent comments
        const { rows: comments } = yield tx.query(`SELECT
      tc.id,
      tc.ticket_id,
      t.title as ticket_title,
 ...
 */
function getUserActivityMetrics(tx, organizationId, targetUserId) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        // Get user info
        const { rows: userRows } = yield tx.query(`SELECT
      u.id, u.email, u.full_name, u.user_type, u.client_id, u.is_active, u.created_at,
      c.name as client_name
    FROM app_user u
    LEFT JOIN client c ON c.id = u.client_id
    WHERE u.id = $1 AND u.organization_id = $2`, [targetUserId, organizationId]);
        if (userRows.length === 0)
            throw (0, errors_1.notFound)('User not found');
        const user = userRows[0];
        const userRole = user.user_type;
        // Get ticket metrics
        const { rows: ticketMetrics } = yield tx.query(`SELECT
      COUNT(*)::int as total,
    ...
    WHERE t.is_deleted = false
      ...
      AND (t.raised_by_user_id = $1 OR t.assigned_to_user_id = $1)`, [targetUserId, organizationId]);
    ...
    GROUP BY s.id, s.name
    ORDER BY s.sequence, s.name`, [organizationId, targetUserId]);
        // Get tickets by priority
        const { rows: byPriority } = yield tx.query(`SELECT
      pr.id as priority_id,
      pr.name as priority_name,
    ...
    WHERE t.is_deleted = false
      ...
      AND (t.raised_by_user_id = $2 OR t.assigned_to_user_id = $2)
    GROUP BY pr.id, pr.name
    ORDER BY pr.rank`, [organizationId, targetUserId]);
        // Get activity metrics
        const { rows: activityMetrics } = yield tx.query(`SELECT
    ...
    WHERE t.is_deleted = false
      ...
      AND (t.raised_by_user_id = $1 OR t.assigned_to_user_id = $1)`, [targetUserId, organizationId]);
        ...
        // Get events by type
        const { rows: eventsByType } = yield tx.query(`SELECT
      te.event_type,
      COUNT(*)::int as count
    FROM ticket_event te
    JOIN ticket t ON t.id = te.ticket_id
    ...
    ORDER BY count DESC`, [targetUserId, organizationId]);
        // Get performance metrics (last 30 days)
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        const { rows: performanceMetrics } = yield tx.query(`SELECT
    ...
    WHERE t.is_deleted = false
      AND c.organization_id = $2
      AND (t.raised_by_user_id = $1 OR t.assigned_to_user_id = $1)`, [targetUserId, organizationId, thirtyDaysAgo]);
        ...
        const { rows: responseTimeRows } = yield tx.query(`SELECT
    ...
    WHERE t.is_deleted = false
      ...
      AND t.created_at >= $3`, [targetUserId, organizationId, thirtyDaysAgo]);
        // Calculate average resolution time (time to close)
        const { rows: resolutionTimeRows } = yield tx.query(`SELECT
    ...
    WHERE t.is_deleted = false
      ...
      AND t.closed_at >= $3`, [targetUserId, organizationId, thirtyDaysAgo]);
        // Get project membership
        const { rows: projectMetrics } = yield tx.query(`SELECT
    ...
    JOIN client c ON c.id = p.client_id
    WHERE pm.user_id = $1 AND c.organization_id = $2`, [targetUserId, organizationId]);
        ...
        return {
            ...
            performance: {
                averageResponseTime: ((_a = responseTimeRows[0]) === null || _a === void 0 ? void 0 : _a.avg_response_hours) ? parseFloat(responseTimeRows[0].avg_response_hours) : undefined,
                averageResolutionTime: ((_b = resolutionTimeRows[0]) === null || _b === void 0 ? void 0 : _b.avg_resolution_hours) ? parseFloat(resolutionTimeRows[0].avg_resolution_hours) : undefined,
